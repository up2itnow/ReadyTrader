...
def test_bus_prefers_fresh_higher_priority(monkeypatch):
    ...
    ws = FakeProvider("exchange_ws", {"symbol": "BTC/USDT", "last": 10, "timestamp_ms": now - 500})
    ingest = FakeProvider("ingest", {"symbol": "BTC/USDT", "last": 11, "timestamp_ms": now - 10})
    rest = FakeProvider("ccxt_rest", {"symbol": "BTC/USDT", "last": 12, "timestamp_ms": now - 5})

    bus = MarketDataBus([ws, ingest, rest])
    res = bus.fetch_ticker("BTC/USDT")
    assert res.source == "ingest"
    assert res.meta["stale"] is False


def test_bus_falls_back_when_provider_errors(monkeypatch):
    now = 1_000_000
    monkeypatch.setattr("marketdata.bus._now_ms", lambda: now)
...
def test_bus_outlier_flag(monkeypatch):
    now = 1_000_000
    monkeypatch.setattr("marketdata.bus._now_ms", lambda: now)
    monkeypatch.setenv("MARKETDATA_OUTLIER_MAX_PCT", "10.0")
    monkeypatch.setenv("MARKETDATA_OUTLIER_WINDOW_MS", "100000")

    p = FakeProvider("ingest", {"symbol": "BTC/USDT", "last": 100, "timestamp_ms": now})
    bus = MarketDataBus([p])
    r1 = bus.fetch_ticker("BTC/USDT")
    assert r1.meta["outlier"] is False

    # 50% jump should be flagged vs last good
    p2 = FakeProvider("ingest", {"symbol": "BTC/USDT", "last": 150, "timestamp_ms": now})
    bus2 = MarketDataBus([p])
    _ = bus2.fetch_ticker("BTC/USDT")
    bus2._providers = [p2]
